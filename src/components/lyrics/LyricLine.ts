...
const getFonts = (isMobile: boolean, customFontSize?: number) => {
  const baseSize = customFontSize || (isMobile ? 32 : 40);
  const transSize = Math.round(baseSize * 0.55); // Proportional to base size
  return {
...
export class LyricLine implements ILyricLine {
  ...
  private drawGlowAnimation(
    ...
    const progress = Math.max(0, Math.min(1, elapsed / duration));
    const { main, mainHeight } = getFonts(this.isMobile, this.customFontSize);
